...
public class AwardPointsEndpoint(ILoyaltyAccountService loyaltyAccountService)
    : Endpoint<AwardPointsRequest, AwardPointsResponse>
{
    public override void Configure()
    {
        Post("api/loyalty-account/award");
        AuthSchemes(JwtBearerDefaults.AuthenticationScheme);
        Description(d =>
        {
            d.Produces<AwardPointsResponse>(StatusCodes.Status200OK);
    ...
    public override async Task HandleAsync(AwardPointsRequest request, CancellationToken ct)
    {
        ...
        try
        {
            var account = await loyaltyAccountService.AwardPointsAsync(customerId, request.Points, request.OrderId, ct);
            var now = DateTimeOffset.Now;

            var response = new AwardPointsResponse(request.CorrelationId())
            {
